...
function love.update()
  if (not game_over) then
    jam_timer = increment(jam_timer, 1 / 30)
  end
  screen:update()
end
...
function new_start_screen()
  ...
  s.update = function(s)
    ...
    if (love.keyboard.isDown("up")) then
      s.selected = max(s.selected - 1, 1)
    end
    if (love.keyboard.isDown("down")) then
      s.selected = min(s.selected + 1, #s.opts)
    end
  ...
  s.draw = function(s)
    love.graphics.clear()
    for i = 1, #s.opts, 1 do
      love.graphics.print(s.opts[i], 50, 60 + (10 * i))
    end
    love.graphics.draw(32, 40, 58 + (10 * s.selected))
    love.graphics.print("gram jam", 40, 20)
    love.graphics.print("the global jam game", 50, 28)
  end
...
function new_credits_screen()
  ...
  s.init = function(s)
    s.opts = {
      "elyjah 'mr ely' wirth",
      "robert 'rantingbob' gardner",
      "mari 'make a jam game?' kyle",
    }
    s.back = false
  end
  ...
  s.draw = function(s)
    love.graphics.clear()
    for i = 1, #s.opts, 1 do
      love.graphics.print(s.opts[i], (5 * i), 60 + (10 * i))
    end
    love.graphics.print("global jam game", 60, 20)
  end
...
function new_gameover_screen()
  ...
  s.init = function(s)
    s.lines = {
      "you have repaired the universe",
      "you have returned all to",
      "the primardial jam",
    }
    s.back = false
    game_over = true
  ...
  s.draw = function(s)
    ...
    for i = 1, #s.lines, 1 do
      love.graphics.print(s.lines[i], (i * 10), (i * 8))
    end
    love.graphics.draw(8, 32, 8, 8, 48, 48, 32, 32)
    local score = "score: " .. (32767 - jam_timer)
    love.graphics.print(score, 64 - (4 * (#score / 2)), 82)
  end
...
function new_game_screen()
  ...
  s.init = function(s)
    ...
    for scene in all(s.scenes) do
      if (scene.unlocked) then
        add(s.active_scenes, scene)
      end
    end
    s.current_scene = 1
  end
  ...
  s.draw = function(s)
    ...
    love.graphics.draw(s.active_scenes[1].icon.x, s.active_scenes[1].icon.y, 32, 16, 0, 104)
    if (s.active_scenes[2]) then
      love.graphics.draw(s.active_scenes[2].icon.x, s.active_scenes[2].icon.y, 32, 16, 32, 104)
    end
    if (s.active_scenes[3]) then
      love.graphics.draw(s.active_scenes[3].icon.x, s.active_scenes[3].icon.y, 32, 16, 64, 104)
    end
    if (s.active_scenes[4]) then
      love.graphics.draw(s.active_scenes[4].icon.x, s.active_scenes[4].icon.y, 32, 16, 96, 104)
    end
    ...
    love.graphics.print(price, dollar_x, 122)
    dollar_x = dollar_x + (#price * 4) + 8
...
function new_upgrade_scene()
  ...
  s.update = function(scene)
    ...
    if scene.active and #scene.available_upgrades > 0 then
      ...
      if (not selected.quantity) then
        selected.quantity = 0
      end
      if
        love.keyboard.isDown("x")
        and can_spend(selected.price, selected.scale)
        and selected.quantity < selected.max_quantity
      then
  ...
  s.draw = function(scene)
    ...
    for i = loop_start, loop_end, 1 do
      ...
      love.graphics.draw(x_pix, y_pix, 8, 8, x_target, y_target, 16, 16)
      love.graphics.print(price, x_target, y_target + 17)
      if (i == scene.selected_upgrade) then
        rect(x_target - 1, y_target - 1, x_target + 16, y_target + 16, 7)
      end
      row = row + 1
      if (row > 4) then
        row = 0
    ...
    for i = 1, #scene.purchased_upgrades, 1 do
      ...
      love.graphics.draw(x_pix, y_pix, 8, 8, x_target, y_target, 16, 16)
      love.graphics.print(upgrade.quantity, x_target + 17, y_target)
    end

    if #scene.available_upgrades > 0 then
      love.graphics.print(scene.available_upgrades[scene.selected_upgrade].name, 0, 1)
      love.graphics.print(scene.available_upgrades[scene.selected_upgrade].description, 0, 96)
    end
...
function new_store_scene()
  ...
  s.update = function(scene)
    ...
    if time() - granimation_time > 1 then
      ...
      granimation = granimation + 4
      if granimation > 136 then
        granimation = 128
      end
    end
    ----
    ...
    for jam in all(scene.stock) do
      if jam.sale_period_counter == nil then
    ...
    if scene.active and #scene.stock > 0 then
      ...
      if love.keyboard.isDown("x") and scene.stock[scene.selected_stock].quantity > 0 then
  ...
  s.draw = function(scene)
    map(scene.background.x, scene.background.y, 0, 0, 16, 16)
    love.graphics.draw(granimation, 20, 24, 4, 4)
    ...
    for i = loop_start, loop_end, 1 do
      ...
      love.graphics.draw(x_pix, y_pix, 8, 8, x_target, y_target, 16, 16)
      love.graphics.print(ing.quantity, x_target, y_target + 16)
      column = column + 1
      column = column % 3
      if (column == 0) then
        row = row + 1
      end
      if (i == scene.selected_stock) then
        rect(x_target - 1, y_target - 1, x_target + 16, y_target + 16, 7)
      end
    end
    ...
    if #scene.stock > 0 then
      ...
      local price = cash_symbols[scene.stock[scene.selected_stock].scale]
        .. get_price(scene.stock[scene.selected_stock])
      local demand = "" .. get_demand(scene.stock[scene.selected_stock]) .. "*"
      love.graphics.print(name, 0, 1)
      love.graphics.print("sale price:", 8, 64)
      love.graphics.print(price, 56 - (#price * 4 + 4), 72)
      love.graphics.print("demand:", 8, 80)
      love.graphics.print(demand, 56 - (#demand * 4), 88)
    end
  end
...
function new_farm_scene()
    ...
    if scene.active then
      ...
      if love.keyboard.isDown("x") then
        ...
        if can_spend(desired_bush.price, desired_bush.scale) then
          ...
          if (not desired_bush.quantity) then
            desired_bush.quantity = 0
          end
  ...
  s.draw = function(scene)
    ...
    for i = loop_start, loop_end, 1 do
      ...
      love.graphics.draw(x_pix, y_pix, 8, 8, x_target, y_target, 16, 16)
      love.graphics.print(price, x_target + 18, y_target + 2)
      if (not bush.quantity) then
        bush.quantity = 0
      end
      love.graphics.print(bush.quantity, x_target + 18, y_target + 12)
      if (i == scene.selected) then
        rect(x_target - 1, y_target - 1, x_target + 16, y_target + 16, 7)
      end
      row = row + 1
      if (row > 4) then
      ...
      love.graphics.print(scene.bushes[scene.selected].name, 0, 96)
    ...
    for i = l_min, l_max, 1 do
      ...
      if (bush.template.harvest_time >= bush.harvest_counter) then
        icon = emptybush_icon
      end
      love.graphics.draw(icon, bush.x, bush.y)
    end
  end
...
function new_kitchen_scene()
  ...
  s.update = function(scene)
    ...
    if scene.active and #scene.available_ingredients > 0 then
      ...
      if
        love.keyboard.isDown("x")
        and #scene.mixer_contents < 3
        and scene.available_ingredients[scene.selected_ingredient].quantity > 0
      then
      ...
      if #scene.mixer_contents ~= 0 then
        ...
        if scene.current_recipe and scene.current_recipe.output then
          scene.current_output = scene.current_recipe.output
        else
          scene.current_output = badjam
        end
        if love.keyboard.isDown("z") then
  ...
  s.draw = function(scene)
    ...
    for i = loop_start, loop_end, 1 do
      ...
      if (i == scene.selected_ingredient) then
        rect(x_target - 1, y_target - 1, x_target + 16, y_target + 16, 7)
      end
    end

    --draw mixer--
    if (scene.mixer_contents[1]) then
      love.graphics.draw(scene.mixer_contents[1].icon(), 8, 8)
    end
    if (scene.mixer_contents[2]) then
      love.graphics.draw(scene.mixer_contents[2].icon(), 24, 8)
    end
    if (scene.mixer_contents[3]) then
      love.graphics.draw(scene.mixer_contents[3].icon(), 40, 8)
    end
    ...
    local icon = 4
    if (scene.current_output) then
      icon = scene.current_output.icon()
    end
    love.graphics.draw((icon * 8) % 128, flr(abs(icon / 16)) * 8, 8, 8, 16, 48, 24, 24)
...
manberry = {
  ...
  current_icon = 1,
  icon = function()
    return manberry.icon_options[manberry.current_icon]
  end,
...
function harvest_bush(bush)
  ...
  if (not bush.template.produce.quantity) then
    bush.template.produce.quantity = 0
  end
...
function has_ingredients(recipe)
  ...
  if #recipe.inputs == 1 then
    if (recipe.inputs[1].quantity == 0) then
      return false
    end
  ...
  if #recipe.inputs == 2 then
    ...
    if (recipe.inputs[2].quantity == 0) then
      return false
    end
    input1 = recipe.inputs[1].quantity - 1
    input2 = recipe.inputs[2].quantity - 1
    if (recipe.inputs[1] == recipe.inputs[2]) then
      input2 = decrement(input2)
    end
  end
  ...
  if #recipe.inputs == 3 then
    ...
    if (recipe.inputs[3].quantity == 0) then
      return false
    end
    input1 = recipe.inputs[1].quantity - 1
    input2 = recipe.inputs[2].quantity - 1
    ...
    if (recipe.inputs[1] == recipe.inputs[3]) then
      input3 = decrement(input3)
    end
    if (recipe.inputs[2] == recipe.inputs[3]) then
      input3 = decrement(input3)
    end
  end

  if input1 < 0 or input2 < 0 or input3 < 0 then
    return false
  else
    return true
  end
...
function lookup_recipe(mixer_contents)
  ...
  if #mixer_contents == 2 then
    for recipecheck in all(temp_recipes) do
      ...
      if recipecheck.inputs[1] == mixer_contents[1] then
        used1 = true
      elseif (recipecheck.inputs[1] == mixer_contents[2]) then
        used2 = true
      end

      if recipecheck.inputs[2] == mixer_contents[1] then
        used1 = true
      elseif (recipecheck.inputs[2] == mixer_contents[2]) then
        used2 = true
      end

      if (used1 and used2) then
        add(temp_matches, recipecheck)
      end
    end
  ...
  if #mixer_contents == 3 then
    for recipecheck in all(temp_recipes) do
      ...
      elseif recipecheck.inputs[1] == mixer_contents[2] then
        used2 = true
      elseif (recipecheck.inputs[1] == mixer_contents[3]) then
        used3 = true
      end

      if recipecheck.inputs[2] == mixer_contents[1] and not used1 then
        used1 = true
      elseif recipecheck.inputs[2] == mixer_contents[2] and not used2 then
        used2 = true
      elseif (recipecheck.inputs[2] == mixer_contents[3] and not used3) then
        used3 = true
      end

      if recipecheck.inputs[3] == mixer_contents[1] and not used1 then
        used1 = true
      elseif recipecheck.inputs[3] == mixer_contents[2] and not used2 then
        used2 = true
      elseif (recipecheck.inputs[3] == mixer_contents[3] and not used3) then
        used3 = true
      end
...
function check_for_unlocks(scene)
  if (not primordialjam.quantity) then
    primordialjam.quantity = 0
  end
  if (primordialjam.quantity >= 1) then
    screen = new_gameover_screen()
  end
